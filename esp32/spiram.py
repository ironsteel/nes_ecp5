...
def test():
  ...
  for i in range(6):
    s.led.on()
    s.hwspi.write(bytearray([0,0xFE,0xFF,0xFF,0x00,i&1])) # 1:show/0:hide OSD
    s.led.off()
    sleep_ms(200)
  ...
  for i in range(24):
    s.led.on()
    s.hwspi.write(bytearray([0,0xFD,0xFF,i//(256//64),i*64+63,ord("*")])) # sets address and writes content
    s.led.off()
  ...
  s.hwspi.write(bytearray([0,0xFD,0xFF,11,20])) # sets address
